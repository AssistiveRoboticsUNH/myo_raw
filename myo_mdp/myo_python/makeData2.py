...
class makeData(object):
        ...
        with open(self.imu_file) as f:
            ...
            for line in f:
                if 'orientation' in line:
                    ...
                    continue
                if writing:
                    x = re.search(r'x:\s([-\.\de]+)', line)
                    y = re.search(r'y:\s([-\.\de]+)', line)
                    z = re.search(r'z:\s([-\.\de]+)', line)
                    w = re.search(r'w:\s([-\.\de]+)', line)
                    if x: fout.write(x.group(1)+',')
                    if y: fout.write(y.group(1)+',')
    ...
    def makeOrientation_new(self):
        fout = open(os.path.join(self.out_path, 'ort_'+self.identifier+'.mat'),'w')
...
def readBag(bagFile, dest_path):
    ...
    ort_l = open(os.path.join(dest_path, 'ort_l.dat'), 'w')
    ort_u = open(os.path.join(dest_path, 'ort_u.dat'), 'w')
...
if __name__ == "__main__":
    ...
    for i,item in enumerate(bagFiles):
        ...
        maker = makeData(os.path.join(dest_path,'emg_u.dat'),
                         os.path.join(dest_path,'imu_u.dat'),
                         os.path.join(dest_path,'ort_u.dat'),
                         'u', dest_path)
        maker.makeEMG()
        maker.makeACC()
        maker.makeGyroscope()
        maker.makeOrientation()
        maker.makeOrientation_new()
        
        maker = makeData(os.path.join(dest_path,'emg_l.dat'),
                         os.path.join(dest_path,'imu_l.dat'),
                         os.path.join(dest_path,'ort_l.dat'),
                         'l', dest_path)
        maker.makeEMG()
        maker.makeACC()
        maker.makeGyroscope()
