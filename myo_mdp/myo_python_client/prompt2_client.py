...
#            imu_data.append(np.hstack((Time, imu_i)))
    ...
    np.savetxt('../data/prompt_l.dat', imu_demos_l[:, -4:], delimiter=',')
    np.savetxt('../data/prompt_u.dat', imu_demos_u[:, -4:], delimiter=',')
...
#        demo.callback(0, 1)
    ...
    if msg.data == 10:
        ...
        if progress and not progress.finished:
            progress.end_game()
        mdp = pickle.load(open('../data1/mdp.pkl'))
        args = {"give_prompt": True,
                "mdp": mdp,
                "id": "new patient",
                "task_type": 1
        ...
        if task != msg.data:
            demo = myo_state2.MyoPrompt2(task_type=1)
            time.sleep(.5)
            demo.callback(0, 1)

            progress = myo_state2.Progress(classifier_pkl='../data1/state_classifier.pkl', **args)
        else:
            progress.reset()
#            progress.subscribeIMU()
        ...
        if progress and not progress.finished:
            progress.end_game()
        mdp = pickle.load(open('../data2/mdp.pkl'))
        args = {"give_prompt": True,
                "mdp": mdp,
                "id": "new patient",
                "task_type": 2
        ...
        if task != msg.data:
            demo = myo_state2.MyoPrompt2(task_type=2)
            time.sleep(.5)
            demo.callback(0, 1)

            progress = myo_state2.Progress(classifier_pkl='../data2/state_classifier.pkl', **args)
        else:
#            progress.reset()
#            progress.subscribeIMU()
            progress = myo_state2.Progress(classifier_pkl='../data2/state_classifier.pkl', **args)
        ...
        if progress and not progress.finished:
            progress.end_game()
        mdp = pickle.load(open('../data3/mdp.pkl'))
        args = {"give_prompt": True,
                "mdp": mdp,
                "id": "new patient",
                "task_type": 3
        ...
        if task != msg.data:
            demo = myo_state2.MyoPrompt2(task_type=3)
            time.sleep(.5)
            demo.callback(0, 1)

            progress = myo_state2.Progress(classifier_pkl='../data3/state_classifier.pkl', **args)
        else:
#            progress.reset()
#            progress.subscribeIMU()
            progress = myo_state2.Progress(classifier_pkl='../data3/state_classifier.pkl', **args)
